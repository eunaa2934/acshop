...
for (let i = 0; i < orderOptions.rows.length; i++) {
  orderOptions.rows[i].addEventListener("click", (event) => {
    ...
    productPictureActive = document.querySelector(
      ".product__pictures.opt" + optionNum,
    );
    productPictureActive.classList.add("active");
  });
}

let optionQuantities = document.querySelectorAll(
  ".order__option__optionQuantity>select",
);
const orderTotalPrice = document.querySelector(".order__total__price");

const orderBtn = document.querySelector(".order__btn");
optionQuantities.forEach((optionQuantity) => {
  optionQuantity.addEventListener("change", () => {
    let sum = 0;
    optionQuantities.forEach((selectedQuantity, index) => {
